...
func TestDbAdd(t *testing.T) {
    ...
    var tests = []struct{data Data; err bool}{
        { d1, false}, // успех
    }
    assert := assert.New(t)
    db, err := NewDatabase()
    ...
    for i, test := range tests {
        ...
        assert.Equal(test.err, err != nil, "Test %d: %v\n", i, err)
    }
}
...
func TestDbGetId(t *testing.T) {
    var tests = []struct{id int32; err bool}{
    ...
    for i, test := range tests {
        ...
        if err != nil {
            assert.Equal(test.err, true, "Test %d\n", i)
            continue
        }
        assert.Equal(test.id, id, "Test %d\n", i)
    }
}
...
func getId(db Database, testId int32) (int32, error) {
    ...
    for rows.Next() {
        ...
        if len(v) == 0 {
            return 0, fmt.Errorf("values len is 0")
        }
        id = v[0].(int32)
    }
